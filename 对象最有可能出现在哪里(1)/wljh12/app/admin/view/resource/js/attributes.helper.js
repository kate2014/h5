...
function reset() {
    ...
    htSkuItems.clear();
    reBindValid();
    $("#ctl00_contentHolder_chkSkuEnabled").attr("checked", false);
...
function prepareAttributes(attributes) {
    ...
    $.each(attributes, function(i, attribute) {
        ...
        if (attribute.UsageMode == "1") {
            var selectTag = $(String.format("<select id=\"attribute{0}\" class=\"form-control input-small formselect input162\" style=\"float:left\"><\/select>", attribute.AttributeId));
        ...
        liTag.append(liLDiv);
        liTag.append($("<div id=\"div_"+attribute.AttributeId+"\" style=\"display:none;float:left;margin-left:10px;\"><input type=\"text\" id=\"txtvalue"+attribute.AttributeId+"\" class=\"form-control\" style=\"width: 135px;display: initial;\"/><input type=\"button\" style=\"margin-bottom:5px;\" class=\"btn btn-primary\" value=\"保存\" onclick=\"javascript:AddValue('"+attribute.AttributeId+"','"+attribute.UsageMode+"',this)\"/></div>"));
        ulTag.append(liTag);
    });
    ...
    $.ajax({
            ...
            success: function(data)
            {
                if (data.Status == "true")
                {
                    ...
                    if(userMode=="1"){
                        $("#attribute"+attributeId).append($(String.format("<option value=\"{0}\">{1}<\/option>",data.msg,valuename)));
                        ...
                        var valItem = $("<span class=\"valspan\"><\/span>")
                        valItem.append($(String.format("<input type=\"checkbox\" name=\"{0}\" id=\"{1}\" valueId=\"{2}\" checked=\"checked\" \/>", checkGroupName, cid, data.msg)));
                        valItem.append($(String.format("<label for=\"{0}\">{1}<\/label>", cid,valuename)))
                        $("li[attributeid='5'] .formitemtitle"+attributeId).after(valItem);
...
function restoreState() {
    ...
    if ($("#ctl00_contentHolder_chkSkuEnabled").attr("checked") == true||$("#ctl00_contentHolder_chkSkuEnabled").attr("checked")=="checked") {
        // 规格值回发状态维护
        enableSku();
...
function prepareSkus() {
    ...
    tableHeader.append($("<td align=\"center\" style=\"width:120px\"><em style='color: #F00;float: left'>*<\/em>现价<br\/>(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:60px\">成本价<br\/>(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:45px;\"><em style='color: #F00;float: left'>*<\/em>库存<\/td>"));
...
function createFieldCell(skuId, skuName) {
    var fieldCell = $(String.format("<td align=\"center\" class=\"fieldCell\" style=\"width:48px\" skuId=\"{0}\"><span>{1}<\/span><\/td>", skuId, skuName));
...
function adjustSkuBox() {
    ...
    skuBox.css("top",(parseInt(skuBox.css("top").replace("px",""))-59)+"px");
...
//    if (!PageIsValid())
//        return false;
    var txtProductName=$("#ctl00_contentHolder_txtProductName");
    var txtMarketPrice=$("#ctl00_contentHolder_txtMarketPrice");
    var txtSalePrice=$("#ctl00_contentHolder_txtSalePrice");
    var txtCostPrice=$("#ctl00_contentHolder_txtCostPrice");
    var txtStock=$("#ctl00_contentHolder_txtStock");
    
    if(txtProductName.val()==""){
        txtProductName.parent().parent().addClass("has-error");
        txtProductName.focus();
        return false;
    }else{
        txtProductName.parent().parent().removeClass("has-error");
    }
    if(txtMarketPrice.val()==""){
        txtMarketPrice.parent().parent().addClass("has-error");
        txtMarketPrice.focus();
        return false;
    }else{
        txtMarketPrice.parent().parent().removeClass("has-error");
    }
    if(txtSalePrice.val()==""){
        txtSalePrice.parent().parent().addClass("has-error");
        txtSalePrice.focus();
        return false;
    }else{
        txtSalePrice.parent().parent().removeClass("has-error");
    }
    if(txtStock.val()==""){
        txtStock.parent().parent().addClass("has-error");
        txtStock.focus();
        return false;
    }else{
        txtStock.parent().parent().removeClass("has-error");
    }
    // 2.如果开启了规格，则做以下检查
    if (skuEnabled) {
        // 商品规格数量需大于等于2
