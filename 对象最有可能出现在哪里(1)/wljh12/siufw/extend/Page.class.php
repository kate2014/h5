<?php
class Page {
    function __construct($rowtotal, $pagesize=30, $url_par=null) {
        return $this->pages($rowtotal, $pagesize);
    }
    
    static function pages($rowtotal, $pagesize=30, $url_par=null) {
    //	echo $_SERVER['REQUEST_URI'];

        $pagecount = intval(($rowtotal+$pagesize-1)/$pagesize); //页数

        $nowpage = isset($_GET['p'])?$_GET['p']:1;
        $prevpage = ($nowpage==1)?$nowpage:$nowpage-1;
        $nextpage = ($nowpage==$pagecount)?$nowpage:$nowpage+1;
        $arr_page['rowtotal'] = $rowtotal;
        $arr_page['pagecount'] = $pagecount;
        $arr_page['nowpage'] = $nowpage;
        
        $arr_page['prevpage'] = $prevpage;
        $arr_page['nextpage'] = $nextpage;

        $first_url = ($nowpage==1)?'javascript:;'
            : '?c='.CONTROL;
        $prev_url = ($nowpage==1)?'javascript:;'
            : '?c='.CONTROL.'&p='.($nowpage-1);
        $next_url = ($nowpage==$pagecount)?'javascript:;'
            : '?c='.CONTROL.'&p='.($nowpage+1);
        $last_url = ($nowpage==$pagecount)?'javascript:;'
            : '?c='.CONTROL.'&p='.$pagecount;
      /*  $first_url = ($nowpage==1)?'javascript:;'
                  : '?c='.CONTROL.'&act='.ACT  ;
        $prev_url = ($nowpage==1)?'javascript:;'
                  : '?c='.CONTROL.'&act='.ACT.'&p='.($nowpage-1);
        $next_url = ($nowpage==$pagecount)?'javascript:;'
                  : '?c='.CONTROL.'&act='.ACT.'&p='.($nowpage+1);
        $last_url = ($nowpage==$pagecount)?'javascript:;'
                  : '?c='.CONTROL.'&act='.ACT.'&p='.$pagecount;*/
        $arr_page['first_url'] = $first_url . $url_par;
        $arr_page['prev_url'] = $prev_url . $url_par;
        $arr_page['next_url'] = $next_url . $url_par;
        $arr_page['last_url'] = $last_url . $url_par;
        
        $arr_page['url_par'] = $url_par;
        
        $arr_page['pagesize'] = $pagesize;
