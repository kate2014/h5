...
function cut_html($string, $sublen){
	$string = strip_tags($string);
	$qian = array(" ","　","\t","\n","\r");
	$string = str_replace($qian, '', $string);
	$string = preg_replace ('/\n/is', '', $string);
 ...
 * @return $rs	array 返回值
 */
 function send_sms($message, $telphone) {
	...
	//    $gateway = "http://ipyy.net/WS/Send.aspx?CorpID={$uid}&Pwd={$passwd}&Mobile={$telphone}&Content={$message}&Cell=&SendTime=";



	$gateway = "http://sz.ipyy.com/sms.aspx?action=send&userid=&account={$uid}&password={$passwd}&mobile={$telphone}&content={$message}&sendTime=&extno=";
...
function guo_img($str){
	preg_match('/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\\1[^>]*?\/?\s*>/i',$str,$match);
	return $match[0];
}
function guo_img_max($str){
	preg_match('/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\\1[^>]*?\/?\s*>/i',$str,$match);
	return $match;
}