...
class UserModel extends BaseModel {
	...
	function getAllCount($where) {
		$sql = "SELECT count(*) FROM " . TABLE_PREFIX . $this->table . " where " .$where;
		$rs = $this->db->select($sql);
		return $rs[0]['count(*)'];
	}
	function getMCount($user_id) {
		$sql = "SELECT count(*) FROM " . TABLE_PREFIX . $this->table . " where id<=" .$user_id;
		$rs = $this->db->select($sql);
		return $rs;
	}
	...
	function getUserByUserName($username) {
		$sql = "SELECT * FROM " . TABLE_PREFIX . $this -> table . " " . "WHERE username='$username' ";
